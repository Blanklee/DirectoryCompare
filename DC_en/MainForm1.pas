...
begin
  ...
  if pauseCompare then begin
    pauseButton.Caption:= '(&C)ontinue';
    pauseButton.Glyph:= tempContButton.Glyph;
  end
  else begin
    pauseButton.Caption:= '(&P)ause';
...
begin
  if (not FileTimeToLocalFileTime(FileTime, LFileTime)) then result:= 'File Date Error' else
  if (not FileTimeToSystemTime (LFileTime, SystemTime)) then result:= 'File Date Error' else
  begin
...
begin
  ...
  curFile.Caption:= 'File: ' + toRelativePath(Cure1(src),srcPath.Text);   //현재 비교하고 있는 파일을 화면에 표시

  totalFiles.tag:= totalFiles.tag + 1;          // 총 Source 파일수를 표시
  totalFiles.Caption:= 'Source Files: ' + inttostr3(totalFiles.tag);
  ...
  repeat                                        // 파일의 내용을 비교한다
    ...
    begin
      ...
      totalBytes.Caption:= 'Bytes Compared: ' + inttostr3(totalBytes64);
      result:= result - [fileEqual];            // 리턴값 = 두 파일이 다르다
...
begin
  ...
  if (fileEqual in r) then begin            // 두 파일이 같으면
    ...
    SameFile.Caption:= 'Same Files: ' + inttostr3(SameFile.tag);
    if OptionForm.C8.Checked then           // 선택사항 - 현재파일 표시
      if OptionForm.C9.Checked              // 선택사항 - 자세히 표시
      then Memo1.Lines.Add ('Same: ' + src + ' == ' + dst)
      else Memo1.Lines.Add ('Same: ' + src);
  ...
  // 파일의 날짜/시각이 서로 다르다면
  if (dateDifferent in r) then s:= 'File Age';

  // 파일의 속성이 서로 다르다면
  if (attrDifferent in r) then
  if (s='') then s:= 'File Attr'
  else s:= 'Age, Attr';

  // 파일의 크기가 서로 다르다면
  if (sizeDifferent in r) then
  if (s='') then s:= 'File Size'
  else begin
    if (s[5]<>',') then delete (s, 1, 5);   // 맨앞에 'File ' 자를 지운다
    // delete (s, length(s)-1, 2);          // 주격조사 '이'를 지운다
    s:= s + ', Size';                       // 끝에 ', Size'를 끼워넣는다
  end;

  // 파일의 대소문자 이름이 서로 다르다면
  if (nameDifferent in r) then
  if (s='') then s:= 'File Name'
  else begin
    if (s[5]<>',') then delete (s, 1, 5);   // 맨앞에 'File ' 자를 지운다
    // delete (s, length(s)-1, 2);          // 주격조사를 지운다
    s:= s + ', Name';                      // 끝에 ', Name'를 끼워넣는다
  end;

  // 도스파일이름이 서로 다르다면
  if (dosnameDifferent in r) then
  if (s='') then s:= 'DOS Name'
  else begin
    if (s[5]<>',') then delete (s, 1, 5);   // 맨앞에 'File ' 자를 지운다
    // delete (s, length(s)-1, 2);          // 주격조사를 지운다
    s:= s + ', DOS Name';                   // 끝에 ', DOS Name'를 끼워넣는다
  end;

  // 파일의 내용이 서로 다르다면
  if (dataDifferent in r) then
  if (s='') then s:= 'File DATA'
  else begin
    if (s[5]<>',') then delete (s, 1, 5);   // 맨앞에 'File ' 자를 지운다
    // delete (s, length(s)-1, 2);          // 주격조사를 지운다
    s:= s + ', DATA';                       // 끝에 ', DATA'를 끼워넣는다
  end;

  if (s > '') then
  if (not OptionForm.C9.Checked) then       // 선택사항 - 자세하게 표시 아니면
    Memo1.Lines.Add (s + ' Differ: ' + src)
  else begin                                // 선택사항 - 자세하게 표시
    Memo1.Lines.Add (s + ' Differ: ' + src +' != ' + dst);
    if (r <> [dataDifferent]) then begin    // 내용만 다를 경우에는 표시하지 않는다.
      s:= '   File1: ';                     // Source 파일의 사항을 자세히 표시
  ...
  if (srcNotFound  in r) then Memo1.Lines.Add ('No File: ' + src) else // 내가 이 프로그램을 잘못 짜지 않는 한 이런 경우는 절대 없을 것이다
  if (srcOpenError in r) then Memo1.Lines.Add ('Cannot Open: ' + src);
  ...
  if (dstNotFound in r) or (dstOpenError in r) then begin
    ...
    notFound.Caption:= 'No Target File:: ' + inttostr3(notFound.tag);
    if (dstNotFound  in r) then Memo1.Lines.Add ('No File: ' + dst) else
    if (dstOpenError in r) then Memo1.Lines.Add ('Cannot Open: ' + dst);
  end;

  // 그외 알수없는 에러 발생시
  if (elseError in r) then Memo1.Lines.Add ('Unknown Error Comparing ' + src);
end;
...
begin
  ...
  if (srcPath.Text = '') then begin
    Memo1.Lines.Add ('Enter Source Directory');
    srcPath.SetFocus; exit;                 // 빠져나간다
  end;

  if (dstPath.Text = '') then begin
    Memo1.Lines.Add ('Enter Target Directory');
  ...
  or (UpperCase(srcFullPath ) = UpperCase(dstFullPath )) then begin
    Memo1.Lines.Add ('Source and Target Directory is Same');
    Memo1.Lines.Add ('Source Directory = ' + Cure1(srcFullPath));
    Memo1.Lines.Add ('Target Directory = ' + Cure1(dstFullPath));
    srcPath.SetFocus; exit;                 // 빠져나간다
  end;
  ...
  Caption:= 'Comparing...';                 // Title바의 글자를 지정하고
  ...
  begin
    Memo1.Lines.Add (Cure1(srcPath.Text) + ' Directory...');
    ...
    Memo1.Lines.Strings[Memo1.Lines.Count-1]:= Cure1(srcPath.Text) + ' Directory: ' + inttoKB(folderSize) + ' (' + inttostr3(folderSize) + ' Bytes)';
    // ProgressBar 뒤에도 표시해 준다.
    folderSizeLabel.Caption:= '[' + inttoKB(folderSize) + ']';
  end;

  DatetimeToString (dt, 'hh:nn:ss', now);
  Memo1.Lines.Add ('['+dt+'] Directory Comparing...');
  ...
  if (OptionForm.C6.Checked) then begin
    Memo1.Lines.Add ('');
    Memo1.Lines.Add ('* No Source File but Exist at Target:');
    ...
    else Memo1.Lines.Strings[Memo1.Lines.Count-1]:= Memo1.Lines.Strings[Memo1.Lines.Count-1] + ' None';
  ...
  else curFile.Caption:= 'File:';       // 정상종료시에는 파일표시를 지운다 (강제로 끝냈으면 냅둔다)
  Caption:= 'Directory Comparer';       // Title바의 글자를 지정하고
  Application.Title:= Caption;          // Minimize했을 때의 Title 글자도 지정한다
  ...
  then Memo1.Lines.Add ('['+dt+'] Directory Compare Completed.')
  else Memo1.Lines.Add ('['+dt+'] Directory Compare Stopped.');

  // 선택사항 - 전체파일 비교율 표시를 하지 않도록 해 놓았으면, 모든 비교가 다 끝난 후에 전체 파일 갯수와 바이트수를 표시해 준다
  if not OptionForm.C12.Checked then
    Memo1.Lines.Add ('Total ' + inttostr3(totalFiles.tag) + ' Files, ' + inttostr3(totalBytes64) + ' Bytes Compared.');

  // 정상적으로 모든 파일을 비교하여 차이점이 발견되지 않았다면 축하 메시지를 하나 던져준다.
  if (stopCompare = false) then
  if (Different = 0) then Memo1.Lines.Add ('Source and Target not differ.');
